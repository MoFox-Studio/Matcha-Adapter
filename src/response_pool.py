...
logger = get_logger("response_pool")
...
async def check_timeout_response() -> None:
    while True:
        ...
        for echo_id, response_time in list(response_time_dict.items()):
            if now_time - response_time > global_config.matcha_server.heartbeat_interval:
        ...
        await asyncio.sleep(global_config.matcha_server.heartbeat_interval)
