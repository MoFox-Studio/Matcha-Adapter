...
logger = get_logger("send_handler")
...
class SendHandler:
    ...
    async def send_normal_message(self, raw_message_base: MessageBase) -> None:
        ...
        if group_info and user_info:
            ...
            return
        logger.info("尝试发送到matcha")
        response = await self.send_message_to_matcha(
            action,
            {
                id_name: target_id,
        ...
        if response.get("status") == "ok":
            ...
            logger.warning(f"消息发送失败，matcha返回：{str(response)}")
    ...
    async def send_command(self, raw_message_base: MessageBase) -> None:
        ...
        response = await self.send_message_to_matcha(command, args_dict)
        if response.get("status") == "ok":
            logger.info(f"命令 {command_name} 执行成功")
        else:
            logger.warning(f"命令 {command_name} 执行失败，matcha返回：{str(response)}")
    ...
    async def handle_adapter_command(self, raw_message_base: MessageBase) -> None:
        ...
        try:
            ...
            # 直接向Napcat发送命令并获取响应
            response = await self.send_message_to_matcha(action, params)
    ...
    async def send_message_to_matcha(self, action: str, params: dict) -> dict:
